...
<div align="center">
  <img src="/readme-resources/image-album-signin.gif" alt="회원가입 및 로그인 시연">
</div>

<div align="center">
  <img src="/readme-resources/image-album-upload-and-download-image.gif" alt="이미지 업로드, 다운로드 및 화면 출력 여부 시연">
</div>
...
- 3 tier architecture 구조를 따라 구성된 웹 서버, 애플리케이션 서버, 데이터베이스 프로그램들을 각각 컨테이너로 격리한 후, 동일한 호스트 위에서 컨테이너들을 실행하는 방식으로 배포 연습을 로컬에서 진행하였습니다. 

<div align="center">
  <img src="/readme-resources/local-web-app-docker.drawio.png" alt="웹앱 구조 및 배포 구조">
</div>
...
server {
  ...
  location /api/ {
    # Docker compose의 app 서비스명을 호스트로 입력.
    # http://<app service name>:<app port>
...
이 repo의 소스 코드를 다음과 같이 다운로드받습니다. 그 후 다운로드 받은 프로젝트 폴더로 이동합니다. 
```shell
git clone https://github.com/JeroCaller/local-docker-web-app-example.git
cd local-docker-web-app-example
```
...
- db-name.txt 파일 내부 예시. 사용할 데이터베이스 이름을 정합니다.

```
image-album-db
```

- spring-datasource-url.txt 예시
  - Docker Compose에서 DB 서비스명이 `db-server`인 경우, url에서도 Docker compose 서비스명 `db-server`를 사용해야합니다. 
  - `jdbc:mariadb://<db service name>:3306/<db name>`

```
jdbc:mariadb://db-server:3306/image-album-db
```
...
# 소스 코드 변경으로 인해 이미지 재빌드해야하는 경우 다음의 명령어를 대신 사용
